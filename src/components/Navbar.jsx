    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "nav-scrolled" : "bg-transparent"}`}>
      ...
      {menuOpen && (
        <div className="lg:hidden border-t"
          style={{ background: "rgba(3,7,18,0.97)", backdropFilter: "blur(20px)", borderColor: "rgba(124,58,237,0.15)" }}>
          <ul className="flex flex-col px-6 py-4 gap-1">
            {navLinks.map((link) => (
              <li key={link.href}>
                <button onClick={() => scrollTo(link.href)}
                  className={`w-full text-left px-4 py-3 rounded-lg text-sm font-medium transition-colors ${
                    active === link.href.replace("#", "") ? "text-violet-400 bg-violet-500/10" : "text-slate-400 hover:text-white hover:bg-white/5"
                  }`}>
                  {link.label}
                </button>
              </li>
