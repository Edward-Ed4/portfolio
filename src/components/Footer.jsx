  ...
  return (
    <footer style={{ borderTop: "1px solid #0d1117", background: "#020c10" }}>
      <div className="max-w-6xl mx-auto px-6 py-16 grid sm:grid-cols-3 gap-12">
        ...
        {/* Navigation */}
        <div>
          <p className="text-xs font-bold tracking-[0.2em] uppercase mb-5" style={{ color: "#06b6d4" }}>
            Navigation
          </p>
          <ul className="space-y-3">
            {navLinks.map((link) => (
              <li key={link.id}>
                <button onClick={() => scrollTo(link.id)} className="text-sm transition-colors duration-200"
                  style={{ color: "#374151" }}
            ...
            {contactInfo.map((c) => (
              <li key={c.label}>
                <a href={c.href} target={c.href.startsWith("mailto") ? undefined : "_blank"}
                  rel="noopener noreferrer" className="text-sm transition-colors duration-200"
                  style={{ color: "#374151" }}
                  onMouseEnter={e => e.currentTarget.style.color = "#22d3ee"}
                  onMouseLeave={e => e.currentTarget.style.color = "#374151"}>
                  {c.label}
      ...
      <div style={{ borderTop: "1px solid #0d1117" }}>
        <div className="max-w-6xl mx-auto px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-xs" style={{ color: "#1f2937" }}>© 2026 Ebaju Edward. All rights reserved.</p>
        </div>
      </div>
    </footer>
  );
